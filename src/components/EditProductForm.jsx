...
export default function EditProductForm({
  ...
  const [newBudjet, setNewBudjet] = useState(budjet);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/products/${id}`, {
        ...
        body: JSON.stringify({ newName, newProject, newImplementation, newEmail, newMobile, newBudjet, newYear, newEvaluation, newWeak, newStrength, newDevelopment, newSuggestion}),
      });
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        />
        <input
          onChange={(e) => setNewBudjet(e.target.value)}
          value={newBudjet}
          className="input input-bordered input-accent w-full max-w-xs"
          type="text"
        />
